...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { amount, description = 'Adicionar Saldo' } = await req.json()

      if (!amount || amount <= 0) {
        return new Response('Invalid amount', { status: 400, headers: corsHeaders })
      }

      // Criar preferência de pagamento no banco
      ...
      if (preferenceError || !preference || preference.length === 0) {
        ...
        return new Response('Failed to create preference', { status: 500, headers: corsHeaders })
      }
      ...
      const mercadoPagoPreference = {
        ...
        back_urls: {
          success: `${req.headers.get('origin') || 'https://jhbafgzfphiizpuoqksj.supabase.co'}/success?payment_id=${preference_id}`,
          failure: `${req.headers.get('origin') || 'https://jhbafgzfphiizpuoqksj.supabase.co'}/failure?payment_id=${preference_id}`,
          pending: `${req.headers.get('origin') || 'https://jhbafgzfphiizpuoqksj.supabase.co'}/pending?payment_id=${preference_id}`
        },
      ...
      if (!response.ok) {
        ...
        return new Response('MercadoPago API error', { status: 500, headers: corsHeaders })
      }
