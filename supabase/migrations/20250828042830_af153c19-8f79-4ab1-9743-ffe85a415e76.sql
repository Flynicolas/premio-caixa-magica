-- Fase 1: Preparação do Banco - Limpeza de CPFs Duplicados

-- 1. Identificar e remover CPFs duplicados (manter apenas o mais recente)
WITH duplicated_cpfs AS (
  SELECT cpf, 
...
SET cpf = NULL 
WHERE cpf IN (
  SELECT cpf FROM duplicated_cpfs WHERE rn > 1
);
