-- Fix the process_scratch_card_game function to use valid transaction types
...
BEGIN
  ...
  -- Criar transação de compra (using 'purchase' instead of 'scratch_purchase')
  ...
  IF p_has_win AND p_winning_item_id IS NOT NULL THEN
    ...
    IF EXISTS (
      ...
      -- Criar transação de prêmio (using 'deposit' instead of 'scratch_prize')
      INSERT INTO public.wallet_transactions (
        user_id, wallet_id, amount, type, description, metadata
      ) VALUES (
