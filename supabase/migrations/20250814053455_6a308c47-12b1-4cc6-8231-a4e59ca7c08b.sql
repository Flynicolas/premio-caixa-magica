-- Fix process_scratch_card_game to use amount_cents instead of amount
CREATE OR REPLACE FUNCTION public.process_scratch_card_game(p_user_id uuid, p_scratch_type text, p_game_price numeric, p_symbols jsonb, p_has_win boolean DEFAULT false, p_winning_item_id uuid DEFAULT NULL::uuid, p_winning_amount numeric DEFAULT 0)
...
BEGIN
  ...
  -- Criar transação de compra - FIXED: usando amount_cents em vez de amount
  ...
  IF p_has_win AND p_winning_item_id IS NOT NULL THEN
    ...
    IF is_money_item AND p_winning_amount > 0 THEN
      ...
      -- Criar transação de prêmio - FIXED: usando amount_cents em vez de amount
      INSERT INTO public.wallet_transactions (
        user_id, wallet_id, amount_cents, type, description, metadata
      ) VALUES (
        p_user_id, user_wallet_id, ROUND(p_winning_amount * 100), 'deposit',
