-- Update play_scratch_rtp function to be RTP-only driven
...
BEGIN
    ...
    IF should_win THEN
        -- Get available prizes that fit in pot
        SELECT array_agg(sp.* ORDER BY sp.prize_value ASC) INTO available_prizes
        ...
        IF array_length(available_prizes, 1) > 0 THEN
            -- Select random prize from available ones
            prize_idx := floor(random() * array_length(available_prizes, 1)) + 1;
            selected_prize := available_prizes[prize_idx];
