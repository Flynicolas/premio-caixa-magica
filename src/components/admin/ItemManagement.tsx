...
import { Plus, Edit, Eye, EyeOff, Package, Calculator } from 'lucide-react';
import AutoWeightHelper from './AutoWeightHelper';
    ...
    <div className="space-y-6">
      <Tabs defaultValue="catalog" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="auto-weights" className="flex items-center gap-2">
            <Calculator className="w-4 h-4" />
            Pesos Automáticos 90/10
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="auto-weights">
          <AutoWeightHelper 
            items={items}
            onRefresh={onRefresh}
          />
...
// Componente separado para o catálogo de itens
const ItemCatalog = ({ items, onRefresh }: ItemManagementProps) => {
  const { toast } = useToast();
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<DatabaseItem | null>(null);
  const [loading, setLoading] = useState(false);
